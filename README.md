...
- 正在播放广播：读取 QQ 音乐、网易云音乐、汽水音乐或酷狗音乐的歌曲信息；支持的客户端可按歌曲比例显示播放进度。QQ 音乐和汽水音乐支持在 ChatBox 第三行显示歌词。
...
- QQ 音乐和汽水音乐现在支持显示歌词，歌词会跟着歌曲播放并显示在 ChatBox 第三行。
- 中文歌曲优先显示简体歌词，减少出现繁体歌词的情况。
- 网易云音乐和酷狗音乐暂不支持歌词。
- 识别到不同音乐软件时，正在播放页面会显示对应的软件图标。
- 修复第一次没有找到歌词后，点击“立即检测”仍不重新查找的问题。

## v2.5.1 更新内容

- 正在播放检测新增汽水音乐与酷狗音乐，并可在网页中自动选择或固定指定播放器。
- 汽水音乐已通过真实 Windows 媒体会话验证，可以读取歌名、歌手、专辑与播放时间轴，并显示动态进度条。
- 酷狗音乐兼容 `KuGou`、`KuGoo`、`KGMusic` 与中文媒体会话标识；进度显示取决于客户端版本是否向 Windows 公开时间轴。
- 继续保留网易云音乐歌曲信息支持；官方客户端不提供有效实时进度时不会发送推算或虚假的进度条。
- 更新播放器状态、等待提示、进度能力说明和相关自动化测试。
...
| 文件名                      | 用途                                                             |
| --------------------------- | ---------------------------------------------------------------- |
| `vrctool-setup-2.5.2.exe` | 安装包，会创建安装记录、快捷方式并配置命令行。 |

安装步骤：

1. 从 [GitHub Releases](https://github.com/danglong0313/vrctool/releases) 下载 `vrctool-setup-2.5.2.exe`。
...
1. 打开 QQ 音乐、网易云音乐、汽水音乐或酷狗音乐并开始播放歌曲。
2. 打开网页 `正在播放` 分区，确认页面显示播放器、歌名和歌手。
3. `自动选择` 会优先使用当前处于播放状态的受支持播放器，也可以固定为 QQ 音乐、网易云音乐、汽水音乐或酷狗音乐。
4. 根据需要调整发送间隔，并使用 `歌名`、`歌手`、`专辑`、`播放器`、`进度`、`歌词` 六个按钮选择广播内容，然后打开 `ChatBox 广播`。

默认开启歌名、歌手和进度，歌词默认关闭。开启歌词后，进度条会在 ChatBox 第二行显示，当前歌词会在第三行显示：
...
`歌名` 表示当前歌曲标题，`歌手` 表示演唱者，`专辑` 表示歌曲所属专辑，`播放器` 表示当前音乐应用，`进度` 表示当前播放时间、文本进度条和歌曲总时长，`歌词` 表示按真实播放位置选择的同步歌词。消息会固定以 `正在播放:` 开头，并且至少需要保留一项广播内容。歌词依赖有效的播放进度，开启歌词时会自动开启进度；播放器没有公开有效时间轴时，不会猜测歌词位置。

歌词目前支持 QQ 音乐和汽水音乐。中文歌曲会优先使用 QQ 音乐歌词，未命中时再尝试其他歌词库，因此更容易显示简体歌词；不会直接修改歌词文字。网易云音乐和酷狗音乐暂不支持歌词。播放器暂停、退出或没有提供有效歌名时，正在播放来源会自动退出 ChatBox 轮播；恢复播放后会自动重新加入。该功能每秒检查一次歌曲状态，但只按页面设置的发送间隔向 ChatBox 广播，不会每次检测都发送。

当前版本识别 QQ 音乐、网易云音乐、汽水音乐与酷狗音乐的 Windows 媒体会话。播放器必须向 Windows 系统媒体控制中心公开播放信息；若页面一直显示等待播放，请先确认 Windows 音量面板或媒体快捷面板能显示该应用的歌曲信息。不同播放器版本可能使用不同的媒体会话标识，可通过页面显示与日志进一步确认适配情况。

网易云音乐进度渠道已经额外评估：Windows 媒体会话是最安全稳定的来源，但网易云当前官方 Windows 客户端返回的开始时间、结束时间和当前位置均为零；本地数据库只保存歌曲元数据与总时长，不保存每秒更新的实时位置；基于 Windows UI Automation 的窗口读取依赖播放器保持可见且主动公开进度控件，网易云的 CEF 界面在托盘模式下不满足条件。现有第三方实现可以通过扫描 `cloudmusic.dll` 并读取进程内存取得进度，但该方案绑定客户端二进制版本、可能触发安全软件，也容易在网易云更新后失效，因此 vrctool 暂不默认集成。后续若网易云官方补齐系统时间轴，现有媒体会话实现会自动使用真实进度。

歌曲信息默认只在本机读取；只有用户主动开启 `歌词` 后，程序才会把歌名、歌手、专辑、歌曲时长或搜索关键词按上述顺序发送给 LRCLIB、QQ 音乐或网易云歌词接口。ChatBox 广播默认关闭，只有用户主动开启后才会发送到 VRChat。
...
vrctool_app/
  ...
  now_playing.py        QQ/网易云/汽水/酷狗媒体会话检测与 ChatBox 广播
